...
var Commands = function (path) {
    ...
    _self.db.run("CREATE TABLE IF NOT EXISTS command (verb TEXT UNIQUE NOT NULL, reply TEXT NOT NULL, params INT NOT NULL, help TEXT, syntax TEXT)");
    ...
    _self.teach = function (params, cb) {
        ...
        _self.db.run("INSERT INTO command (verb, reply, params, help, syntax) VALUES (?, ?, ?, ?, ?)", verb, reply, req, help, syntax, function (err) {
            if (err) {
                console.error(err);
...
Commands.prototype.parse = function (user, message, cb) {
    ...
    var _self   = this;
    var command = message.slice(1).split(" "); // needs better paramters splitting!
    var verb    = command[0];
