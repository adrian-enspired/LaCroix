...
// TODO:
// {me} operator, first test, split off like calls
// split function calls off

var Commands = function (path, config) {

    this.builtin = require('./commands.json');
    ...
    // change to prototype
    this.dispatch = function (reply, params, cb) {
        ...
        _self.isPermitted(cmd.user, cmd.permit, function (permitted) {
            ...
            if (type === "!") {
                ...
                if (typeof cmd.call !== 'undefined') {
                    ...
                    //_self[cmd.call](cmd, function (err, vars, pmforce) {
                        if (err) return cb(err, true);
