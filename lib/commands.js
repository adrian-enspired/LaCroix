...
var Commands = function (path, config) {
    ...
    this.db.run("CREATE TABLE IF NOT EXISTS memo (recipient TEXT UNIQUE NOT NULL, sender UNIQUE NOT NULL, message NOT NULL)");
};
...
// TODO: start with basic HTTP matching, evolve to include any number of optional regex matches
Commands.prototype.auto = function (message, cb) {
    var match = message.match(/(?:https?:\/\/)?(?:[\w|-]+\.)(?:\.?[\w]{2,})+/);
    if (match !== null) {
        request(match[0], function (err, res, body) {
            ...
            if (typeof body !== 'undefined') {
                ...
                if (title.trim() !== '')
                    return cb("[" + type + "] " + $('title').text());
            }
        });
    }
};

// TODO: merge with auto
Commands.prototype.getMemo = function () {
