...
module.exports = {
    ...
    memo : function (cmd, cb) {
        ...
        var msg = cmd.args[1];
        if (msg !== "none") {
            this.db.run("REPLACE INTO memo (tofrom, sender, recipient, message) VALUES (?, ?, ?, ?)", to + cmd.sender, cmd.sender, to, msg, function (err) {
                if (err) return cb("Error performing command.");
                return cb(null, {
            ...
            this.db.run("DELETE FROM memo WHERE tofrom = ?", to + cmd.sender, function (err) {
                if (err) return cb("Error performing command.");
                return cb(null, {
                    'private': [ "removed", to ]
                });
    ...
    notifyme : function (cmd, cb) {
        if (cmd.args[0] !== "none")  {
            this.db.run("REPLACE INTO notify (name, email) VALUES (?, ?)", cmd.sender, cmd.args[0], function (err) {
                if (err) return cb("Error performing command.");
                return cb(null, {
                    'private': [ "can now" ]
                });
            ...
            this.db.run("DELETE FROM notify WHERE name = ?", cmd.sender, function (err) {
                if (err) return cb("Error performing command.");
                return cb(null, {
                    'private': [ "can no longer" ]
                });
            });
        }
    },
    ...
    poke : function (cmd, cb) {
        ...
        _self.db.get("SELECT * FROM notify WHERE name = ?", cmd.args[0], function (err, row) {
            if (err) return cb("Error performing command.");
            if (typeof row === 'undefined') return cb("User has not allowed notification by email.");
            _self.email(row.email, "notifyme@donotreply.com", "Notification from " + cmd.bot, message, function (err, response) {
